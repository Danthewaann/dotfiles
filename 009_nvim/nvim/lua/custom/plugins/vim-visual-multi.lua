return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>vj", "<Plug>(VM-Add-Cursor-Down)", { desc = "[V]isual multi add cursor down" })
    vim.keymap.set("n", "<leader>vk", "<Plug>(VM-Add-Cursor-Up)", { desc = "[V]isual multi add cursor up" })
  end
}
